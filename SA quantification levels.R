...
p = ggplot(df, aes(x = genohpi, y = intercellular, fill= "white")) + 
  ...
  xlab("") + scale_y_continuous(limits = c(0,600), expand = c(0,0)) + ylab("") +  scale_x_discrete(labels = c("Untreated\n", "Mock\n", "Pst\n", "Pst ΔalgU\nmucAB",  "Pst ΔalgU\nmucAB ΔalgD"))  +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.title.y = element_text(size=15),
        axis.text = element_text(colour = "black", size=15),
        axis.text.x = element_text(vjust = -0.25, face = c("plain", "plain", "italic", "italic", "italic")),
        legend.position = "none"
  )
p
anovaModel = aov(intercellular ~ genohpi, data = df)
print(HSD.test(anovaModel, alpha=0.05, "genohpi", console=F)$groups)

ggsave(file = "PTI-IWF-23-1_byStrain-600.svg", plot = p, width = 8, height = 6)


setwd("C:\\Users\\garre\\OneDrive\\Documents\\Cameron Lab- McMaster University\\Data\\Data-PTI Paper 2019\\Exp-Triple and Quad\\IWF SA levels")
