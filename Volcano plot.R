...
tiff(filename = "Volcano_24h.tiff", height = 1000, width = 1000)
ggplot(data = data, aes(x = log2FoldChange, y = -log10(padj), col = de, label = delabel)) + geom_point(size = 2) +
  geom_text_repel(col = "black") +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          ...
          axis.ticks=element_line(colour = "black", size =1), axis.ticks.length = unit(5,"points"),
          axis.title.y = element_text(size=15), legend.position = "right",
          axis.text = element_text(size=10)) +
    geom_vline(xintercept=c(-0.585, 0.585), col="red") + geom_hline(yintercept=-log10(0.05), col="red")+ scale_color_manual(values=c("blue", "black", "red"))+ ylim(0,10)
dev.off()
